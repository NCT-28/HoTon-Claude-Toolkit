...
if [ -d "${PROJECT_DIR}/graphtr-out" ]; then
  GRAPHTR_RULE=" (3b) graphtr-out/ EXISTS: run graphtr-out/query.py query \"<keyword>\" (or path/explain) to scope down to a small subgraph BEFORE serena deep-reads (graphtr -> serena)."
fi
